  ...
  <div class="userTypeManager">
    ...
    <div style="margin: 20px">
      <el-table :data="tableData"
        ...
        <el-table-column prop="type" label="汽车类别" width="220" />
    ...
    >
      <el-form :rules="rules" :model="newOrEditForm" label-width="100px">
        <el-form-item label="汽车类别" prop="type">
          <el-input
              style="width: 300px; margin: 3px 0"
              v-model="newOrEditForm.form.type"
              autocomplete="off"
          ></el-input>
        </el-form-item>
      </el-form>
...
const submitNewOrEdit = () => {
  if (isEdit.value) {
    request.post("/carType/chgCarType", newOrEditForm.form).then((res: any) => {
      if (res.code === 200) {
    ...
    request.post("/carType/insertOne", newOrEditForm.form).then((res: any) => {
      if (res.code === 200) {
...
//  删除
const del = (ids: []) => {
  request.post("/carType/delMore", ids).then((res: any) => {
    if (res.code === 200) {
